...
export const SELECT_NODE_GROUPS = `
  SELECT 
    ...
    COUNT(ngm.MPG_MNG_DOM_NODE_ID) AS "nodeCount",
    LISTAGG(ngm.MPG_MNG_DOM_NODE_ID, ',') WITHIN GROUP (ORDER BY ngm.MPG_MNG_DOM_NODE_ID) AS "nodeIdsStr"
  FROM TWAA0004M00 ng
  LEFT JOIN TWAA0005M00 ngm ON ng.MNG_DOM_NODE_GRP_ID = ngm.MPG_MNG_DOM_NODE_GRP_ID
...
export const SELECT_NODE_GROUP_DETAIL = `
  ...
  FROM TWAA0004M00 ng
  LEFT JOIN TWAA0005M00 ngm ON ng.MNG_DOM_NODE_GRP_ID = ngm.MPG_MNG_DOM_NODE_GRP_ID
  LEFT JOIN TWAA0001M00 n ON ngm.MPG_MNG_DOM_NODE_ID = n.MNG_DOM_NODE_ID
...
export const INSERT_NODE_GROUP_MEMBER = `
  INSERT INTO TWAA0005M00 (
    ...
    (SELECT NVL(MAX(MNG_DOM_NODE_GRP_MMB_MPG_ID),0)+1 FROM TWAA0005M00),
    :nodeGroupId,
    :nodeId,
    'system',
    SYSTIMESTAMP,
...
export const DELETE_NODE_GROUP_MEMBER = `
  ...
  WHERE MPG_MNG_DOM_NODE_GRP_ID = :nodeGroupId AND MPG_MNG_DOM_NODE_ID = :nodeId
`;
...
export const DELETE_ALL_NODE_GROUP_MEMBERS = `
  DELETE FROM TWAA0005M00
  WHERE MPG_MNG_DOM_NODE_GRP_ID = :nodeGroupId
`;
